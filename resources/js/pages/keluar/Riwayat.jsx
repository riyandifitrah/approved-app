...
export default function Riwayat() {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get("/api/transactions/keluar", {
  ...
  const handleDelete = async () => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete("/api/transactions/keluar/delete", {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
  ...
  const handleTambah = () => {
    history.push("/barang-keluar/form");
  };

  const handleEdit = (nomtrans) => {
    history.push(`/barang-keluar/detail/${nomtrans}`);
  };
  const handleApprove = async (nomtrans) => {
    try {
      await axios.put(`/api/transactions/keluar/update-status/${nomtrans}`, { status: 'approved' }, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  const handleReject = async (nomtrans) => {
    ...
    if (catatan) {
      try {
        await axios.put(
          `/api/transactions/keluar/update-status/${nomtrans}`,
  ...
  const columns = [
    { name: "Nomtrans", selector: row => row.nomtrans, sortable: true, wrap: true},
    ...
    <div className="container mt-5">
      <div className="card text-white mb-4 gradient-card">
        <div className="card-body text-center py-4">
          <h1 className="card-title mb-0">
            <i className="bi bi-file-earmark-medical"></i>&nbsp;Data Barang Keluar
          </h1>
        </div>
      </div>
      ...
      <div className="card custom-card mb-5">
        <div className="card-header gradient-header text-white d-flex justify-content-between align-items-center">
          <h5 className="mb-0">
            <i className="bi bi-archive-fill"></i>&nbsp;List Transaksi Keluar
          </h5>
          <div>
