...
export default function Notifikasi() {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get("/api/transactions/keluar/notif", {
  ...
  const handleDelete = async () => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete("/api/transactions/keluar/notif/delete", {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      ...
      <div className="card text-white mb-4 gradient-card">
        <div className="card-body text-center py-4">
          <h1 className="card-title mb-0">
            <i className="bi bi-bell-fill"></i>&nbsp;Notifikasi Barang Keluar
          </h1>
        </div>
      </div>
